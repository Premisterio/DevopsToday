...
The project is organized as a **monorepo** with separate `be/` and `fe/` directories — each with its own Dockerfile, dependencies, and README. This keeps the setup simple: one clone, one `docker compose up`, and everything runs.
...
- **Swagger UI:** <http://localhost:8000/docs> (interactive, auto-generated by FastAPI)
- **Postman Collection:** [`postman/Travel_Planner_API.postman_collection.json`](postman/Travel_Planner_API.postman_collection.json) — import into Postman to test all endpoints with automated assertions. See [postman/README.md](postman/README.md) for instructions.
